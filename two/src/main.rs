...
fn main() {
    ...
    for line in contents.iter() {
        ...
        let (lowest_number, highest_number, character, password): (u8, u8, &str, &str) = (
            my_5_item_vec[0].parse().unwrap(),
            my_5_item_vec[1].parse().unwrap(),
            my_5_item_vec[2],
            my_5_item_vec[4],
        );
        ...
        // println!("{:?} {} {} {} {} {}", my_5_item_vec, lowest_number, highest_number, character, password, password.matches(character).count());
        if (char_vec[low_entry] == character_char_vec[0])
            ^ (char_vec[high_entry] == character_char_vec[0])
        {
            println!(
                "{:?} {} {} {} {} {}",
                my_5_item_vec,
                ...
                character,
                password,
                password.matches(character).count()
            );
            number_valid_two += 1;
        }
    }
